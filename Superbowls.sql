/*Code created as part of excercise*/

/* Superbowls Dataset taken from Khan academy*/
...
/*Return all the data from State florida*/
SELECT * FROM superbowls WHERE state LIKE "Florida";

/*Return count of Florida*/
SELECT COUNT(*) FROM superbowls WHERE state LIKE "Florida";

/*Return the average of winner points*/
SELECT AVG(Winner_Pts) FROM superbowls;

/*Return min of Loser points*/
SELECT MIN(Loser_Pts) FROM superbowls;

/*Return all the winner points per state*/
SELECT Winner, SUM(Winner_Pts), State AS Total_winner_pts
FROM superbowls GROUP BY State 
HAVING Total_winner_pts>5;

/*Return count of each states winner points level*/
SELECT State,COUNT(*), 
	CASE 
    	WHEN Winner_Pts<25 THEN "Satisfactory Winner Points(Points <25)"
        WHEN Winner_Pts<40 THEN "Good Winner Points (Points <40)"
        ELSE "Very Good Winner Points(Points >40)"
    END AS "Winner Points assesment"
FROM superbowls
GROUP BY State ORDER BY COUNT(*);

/*Return the winners if the Winner is either "New England Patriots" or "Denver Broncos"*/
SELECT Winner, Winner_Pts, State FROM superbowls WHERE Winner LIKE "New England Patriots" OR "Denver Broncos";
